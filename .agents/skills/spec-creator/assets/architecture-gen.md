...
系统将给出一组**推荐的模块类型**，用于说明各模块的职责边界与协作关系。

**以下为一组示例模块类型（仅作为范例）：**

* Adapter Module：负责对接并隔离外部系统，作为内部调用的统一入口
* Core Module：提供与业务无关的基础能力或通用功能
* Service Module：承载主要业务流程与状态规则
* Component Logic Module：负责组件级逻辑与状态组织，不涉及展示
* Component View Module：负责组件的展示结构与交互呈现
* Component Module：组合逻辑模块与视图模块，形成可复用的组件单元
...
## Q3. 模块依赖方向（Dependency Rules）— 强制规则
...
* 模块依赖只能**单向指向被允许的模块类型**
* Adapter Module 不依赖任何内部模块
* Core Module 永远不知道 Service Module 的存在
* Component View Module 不得包含任何逻辑判断
* Component Module 不得跨模块类型直接调用
...
## Q4. 目录与模块落点规则（Project Layout Rules）
